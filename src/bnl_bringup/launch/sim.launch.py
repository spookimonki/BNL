...
def _preflight_no_duplicate_nodes(context, *args, **kwargs):
    ...
    if collisions:
        msg = (
            "BNL sim refused to start because nodes already exist (likely a previous session is still running):\n"
...
def generate_launch_description():
    default_world = PathJoinSubstitution([FindPackageShare('bnl_sim'), 'worlds', 'example.sdf'])
    default_models = PathJoinSubstitution([FindPackageShare('bnl_sim'), 'models'])
    ...
    default_rviz_config = PathJoinSubstitution(
        [FindPackageShare('bnl_navigation'), 'config', 'rviz_nav2_slam.rviz']
    )
    ...
    layer3_nav2 = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            PathJoinSubstitution([FindPackageShare('bnl_navigation'), 'launch', 'bnl_layer3_nav2.launch.py'])
        ),
